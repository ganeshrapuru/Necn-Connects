...
const addStory = async (req, res) => {
  ...
  try {
    const query = `
      ...
      VALUES (?, ?, ?, ?, ?, ?)
    `;
    await pool.query(query, [story_name, story_description, content, author, suggested_skill, thumbnail]);
...
const updateStory = async (req, res) => {
  ...
  try {
    const query = `
      ...
      WHERE story_id = ?
    `;
    const [result] = await pool.query(query, [story_name, story_description, content, suggested_skill, thumbnail, story_id]);
...
const getStory = async (req, res) => {
  ...
  try {
    const query = `
      ...
      WHERE s.story_id = ?
    `;
    const [story] = await pool.query(query, [story_id]);
  ...
  try {
    const query = `
      ...
      WHERE 
          s.author = ?;
    `;
    const [stories] = await pool.query(query, [author_id]);
